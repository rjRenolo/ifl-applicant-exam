...
<div>
    ...
    <v-container v-else-if="!this.englishDone">
      <!-- <span>
        item : {{this.$store.getters.getAnsweredMath.length}}
        <br />out of: 20
      </span>-->
      ...
      <v-row justify="center">
        ...
        <v-col v-if="this.currentQuestion.choices.length === 5" cols="8" sm="6" md="3" style="display:flex; justify-content:center;">
          <v-btn color="primary" outlined @click="myAnswerIs('d')">
            <span style="color:black !important">{{this.currentQuestion.choices[4]}}</span>
          </v-btn>
        </v-col>
      </v-row>
      <v-row justify="center">
...
export default {
    ...
    firestore() {
      return {
        questions: db.collection("exam").doc("math")
      };
    },
    methods:{
      myAnswerIs(letter) {
      ...
      answerSubmitted() {
        ...
        if (checkingAns === this.currentQuestion.correct) {
          ...
          this.$store.commit("CORRECT_ANSWER_INCREMENT_MATH");
          db.collection("applicants")
            .doc(this.$store.getters.getApplicantId)
            .update({ resultMath: this.$store.getters.getResultMath })
            .then(() => {
              this.$store.commit(
                "PUSH_ANSWERED_QUESTION_MATH",
                this.currentQuestionIndex
              );
              var flag = true;
              if (this.$store.getters.getAnsweredMath.length < 20) {
                while (flag) {
                  let randomize = Math.floor(Math.random() * 20);
                  if (
                    !this.$store.getters.getAnsweredMath.includes(randomize)
                  ) {
                    flag = false;
              ...
              db.collection("applicants")
                .doc(this.$store.getters.getApplicantId)
                .update({
                  answeredMath: this.$store.getters.getAnsweredMath
                });
            })
            .catch(error => {
              console.log(error);
          ...
          // nope answer
          this.$store.commit("PUSH_ANSWERED_QUESTION_MATH", this.currentQuestionIndex);
          db.collection("applicants")
            .doc(this.$store.getters.getApplicantId)
            .update({ answeredMath: this.$store.getters.getAnsweredMath });
          var flag = true;
          if (this.$store.getters.getAnsweredMath.length < 20) {
            while (flag) {
              let randomize = Math.floor(Math.random() * 20);
              if (!this.$store.getters.getAnsweredMath.includes(randomize)) {
                flag = false;
                this.currentQuestionIndex = randomize;
                this.currentQuestion = this.questions.questions[
                  this.currentQuestionIndex
    ...
    watch: {
      questions: function() {
        if (
          !this.currentQuestionIndex &&
          this.$store.getters.getAnsweredMath.length < 20
        ) {
          this.currentQuestionIndex = 0;
          if (
            this.$store.getters.getAnsweredMath.includes(
              this.currentQuestionIndex
            )
          ) {
            var flag = true;
            while (flag && this.$store.getters.getAnsweredMath.length < 20) {
              let randomize = Math.floor(Math.random() * 20);
              if (!this.$store.getters.getAnsweredMath.includes(randomize)) {
                flag = false;
                this.currentQuestionIndex = randomize;
                ...
                // this.$store.commit("PUSH_ANSWERED_QUESTION_MATH", randomize);
              }
            }
          } else {
            this.currentQuestion = this.questions.questions[
