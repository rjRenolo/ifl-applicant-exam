<template>
  <v-container> 
      ...
      <v-row>
        ...
        <v-col v-if="addChoices.length === 4">
          <v-btn color="info" v-on:click="addExtraChoice()">Add Extra Choice</v-btn>
        </v-col>
      </v-row>
    ...
    <v-row>
      <v-list>
        ...
        >
          ...
          <v-list-item>
            <v-list-item-content>
              <v-list-item-title>Correct Answer : {{question.correct === 0 ? 'A' : question.correct === 1 ? 'B' : question.correct === 2 ? 'C' : question.correct === 3 ? 'D' : question.correct === 4 ? 'E' : null}}</v-list-item-title>
            </v-list-item-content>
          </v-list-item>
        </v-list-group>
      </v-list>
...
export default {
    ...
    data() {
      return {
        ...
        v => (v.includes('a') || v.includes('b') || v.includes('c') || v.includes('d') || v.includes('e')) || 'Correct Answer is required',
        ],
        snackbar: false,
    ...
    methods: {
      addExtraChoice(){
        this.addChoices.push("e. ");
      },
      submitQuestion(){
        const ans = this.addCorrect === 'a' ? 0 :this.addCorrect === 'b' ? 1 :this.addCorrect === 'c' ? 2 :this.addCorrect === 'd' ? 3 :this.addCorrect === 'e' ? 4 : null;
        const pushMe = {
        ...
        db.collection('exam').doc('math').update({...this.questions})
          .then(() => {
            // console.log('updated')
